/*
OPCloud Single-Boot Toolkit (mouse-only, no direct HeaderComponent access)

Purpose:
- create exactly one Object / Process on a blank canvas
- avoid duplicate creation by NOT firing synthetic DOM drop
- avoid ghosting by following the real mouse lifecycle more closely
- capture and cache InitRappidService during first-object bootstrap by simulating the real add-state UI click
- provide state helpers that use the real semantic addStateAction(visual, init) path
- provide cleanup for toolkit-created items

Globals exposed:
- __opcloudSingleBoot
- createSingleObjectIfMissing
- createSingleProcessIfMissing
- cleanupSingleBoot
*/
...
(async function () {
    ...
    function inspectStateNameBindings(target) {
        ...
        const cellStates = orderedStates.map((s, i) => ({
            index: i,
            id: getIdFromModel(s),
            label: getLabelFromModel(s),
            position: getPositionFromModel(s)
        }));

        const visualStates = Array.isArray(visual?.states)
        ...
        let logicalStates = null;
        try {
            if (typeof logical?.states === "function") {
                const arr = logical.states();
                logicalStates = Array.isArray(arr)
                    ? arr.map((s, i) => ({
                        index: i,
                        id: s?.id,
                        name: s?.name,
                    ...
                    : arr;
            } else if (Array.isArray(logical?.states_)) {
                logicalStates = logical.states_.map((s, i) => ({
                    index: i,
                    id: s?.id,
                    name: s?.name,
                    label: s?.label,
        ...
        const report = {
            ...
            logicalCtor: logical?.constructor?.name ?? null,
            cellStates,
            visualStates,
            logicalStates
        };

        log("inspectStateNameBindings", report);
        return report;
    }

    function inspectEssenceBindings(target) {
        const model = typeof target === "string" ? findObjectByLabel(target) : target;
        if (!model) throw new Error("Target object not found");
        ...
        const report = {
            ...
            textForListLogical: logical?.textForListLogical,
            logicalName: logical?.name
        };

        log("inspectEssenceBindings", report);
        return report;
    }
    ...
    async function createSingleProcessIfMissing(opts = {}) {
        return createSingleKindIfMissing("process", opts);
    }
    ...
    function getCurrentUnfoldActionObject() {
        return BOOT.capturedUiSeed.init?.elementToolbarReference?.unfoldHandle?.action || null;
    }

    function canDirectCleanUnfold(action) {
        if (!action) return false;
        if (typeof action.unfoldAction === "function") return true;
        if (typeof action.unfold === "function") return true;

        const init = BOOT.capturedUiSeed.init;
        const thingId = action?.thing?.id ?? null;
        const cell = thingId && init?.graph?.getCell ? init.graph.getCell(thingId) : null;
        if (cell && typeof cell.unfoldAction === "function") return true;
        return false;
    }

    function inspectUnfoldApi() {
        const action = getCurrentUnfoldActionObject();
        const proto = action ? Object.getPrototypeOf(action) : null;
        const init = BOOT.capturedUiSeed.init;
        const thingId = action?.thing?.id ?? null;
        const cell = thingId && init?.graph?.getCell ? init.graph.getCell(thingId) : null;
        const cellProto = cell ? Object.getPrototypeOf(cell) : null;
        const report = {
            actionCtor: action?.constructor?.name ?? null,
            ownKeys: action ? Object.keys(action) : [],
            protoKeys: proto ? Object.getOwnPropertyNames(proto) : [],
            canDirectCleanUnfold: canDirectCleanUnfold(action),
            hasUnfoldAction: !!(action && typeof action.unfoldAction === "function"),
            hasUnfold: !!(action && typeof action.unfold === "function"),
            thingId,
            cellCtor: cell?.constructor?.name ?? null,
            cellHasUnfoldAction: !!(cell && typeof cell.unfoldAction === "function"),
            cellProtoKeys: cellProto ? Object.getOwnPropertyNames(cellProto) : []
        };
        log("inspectUnfoldApi", report);
        return report;
    }
    ...
    function listOpdTreeNodes() {
        ...
        const rows = opds.map(opd => {
            ...
            return {
                ...
                childCount: Array.isArray(opd?.children) ? opd.children.length : null,
                treeNodeFound: !!node,
                treeNodeText: node?.data?.name ?? node?.data?.text ?? node?.displayField ?? null,
                isActive: !!(node?.isActive),
                isExpanded: !!(node?.isExpanded)
            };
        });
        log("listOpdTreeNodes", rows);
        return rows;
    }
    ...
    function installUnfoldProbeOnTarget(target) {
        ...
        cell.unfoldAction = function (...args) {
            const before = snapshotRuntimeState();
            log("unfoldProbe.before", {
                targetId: getIdFromModel(model),
                targetLabel: getLabelFromModel(model),
                args,
                beforeCurrentOpd: before.currentOpd,
                beforeOpds: before.opds,
                beforeGraphCells: before.graphCells
            });
            ...
            Promise.resolve().then(() => {
                ...
                log("unfoldProbe.after", {
                    ...
                    afterOpds: after.opds,
                    afterGraphCells: after.graphCells,
                    diff: diffRuntimeSnapshots(before, after),
                    unfoldedContents: listCurrentOpdContents(),
                    treeNodes: listOpdTreeNodes()
                });
            });
    ...
    window.__opcloudSingleBoot = {
        ...
        summarizeOpd,
        summarizeGraphCell,
        getCurrentGraphCells,
        snapshotRuntimeState,
        diffRuntimeSnapshots,
        getCurrentUnfoldActionObject,
        canDirectCleanUnfold,
        inspectUnfoldApi,
        unfoldTargetAio,
        summarizeVisualElement,
        listCurrentOpdContents,
        classifyUnfoldScaffold,
        getTreeRuntime,
        listOpdTreeNodes,
        ...
        mapKindToType,
        getExistingModelByKind,
        findNewestCreatedModel,
        getObjectDom,
        getStencilButton,
        getStencilDragSource,
        getFloatingStencilGhosts,
        getLikelyDropTargets,
        chooseDropTarget,
        getToolbarButtons,
        makeDataTransfer,
        fireDragEvent,
        ...
        cleanupGhostUi,
        looksLikeInitRappidService,
        installUiSeedCaptureOn,
        captureUiSeedFromObject,
        ensureUiSeedCaptured,
        createSingleKindIfMissing,
        createSingleObjectIfMissing,
        createSingleProcessIfMissing,
        createSingleObjectFresh,
        createSingleProcessFresh,
        ensureNamedStateInputs,
        renameStatesByVisualOrder,
        addStates,
        updateStatesAio,
        parseEssenceSpec,
        renameNodeAio,
    ...
    window.createSingleObjectIfMissing = createSingleObjectIfMissing;
    window.createSingleProcessIfMissing = createSingleProcessIfMissing;
    window.cleanupSingleBoot = cleanupSingleBoot;
    window.deleteNodeAio = deleteNodeAio;
    window.updateStatesAio = updateStatesAio;
    window.renameNodeAio = renameNodeAio;
    window.setEssenceAio = setEssenceAio;
    ...
    window.unfoldTargetAio = unfoldTargetAio;
    window.inspectUnfoldApi = inspectUnfoldApi;
    window.listCurrentOpdContents = listCurrentOpdContents;
    window.classifyUnfoldScaffold = classifyUnfoldScaffold;
    window.listOpdTreeNodes = listOpdTreeNodes;
